...
class MainWindow(QMainWindow):
    ...
    def _build_fw_params(self) -> FixedWingParameters:
        ...
        return FixedWingParameters(
            cruise_speed_mps=fp.get('speed', 18.0),
            mission_altitude_m=fp.get('altitude', 100.0),
            turn_radius_m=fp.get('turn_radius', 50.0),
                ...
                if scan_mode in ('spiral', 'circular') and self.circle_center is not None:
                    ...
                    if scan_mode == 'spiral':
                        ...
                        mission_latlon = _cov.plan_spiral_from_center(
                            center_lat, center_lon, _max_r,
                            fw_params.scan_spacing_m,
                            is_fixed_wing=True,
                            turn_radius=fw_params.turn_radius_m,
                            curvature=_sp_curv,
                            alt_step=_sp_alt_step,
                            base_alt=_alt,
                            entry_bearing_deg=approach_bearing,
                        )
                        _fw_sp_alts = _cov.get_spiral_altitudes()
                        if _fw_sp_alts and len(_fw_sp_alts) == len(mission_latlon):
                            _mission_path = [(lat, lon, a) for (lat, lon), a in zip(mission_latlon, _fw_sp_alts)]
                        else:
                        ...
                        mission_latlon = _cov.plan_circle_survey_from_center(
                            ...
                            fw_params.scan_spacing_m,
                            base_alt=_alt,
                            direction=fw_params.circle_direction,
                            min_radius_m=_min_r,
